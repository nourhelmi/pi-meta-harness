...
Give the helper the path to its matching role skill and the packet; it reads that role and its common contract. Do not preload every role yourself. Shipped roles: `advisor-role-builder`, `advisor-role-advisor`, `advisor-role-checker`, `advisor-role-browser-verifier`. Browser-verifier is a focused, economical worker for explicit browser scenarios or before/after evidence with a small task-shaped context; makers still own ordinary browser checks. Use an inline bounded contract for a genuinely mixed task, not to evade role boundaries. These are instructions, not automatically registered native agent types: use an available native agent type and supply the role skill path in its prompt.
