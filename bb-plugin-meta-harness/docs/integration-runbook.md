...
2. Use the actual packaged native build produced from the reviewed source. Have Node 24+, an installed Codex CLI whose executable identity, App Server handshake, protocol, and effective-config preflight pass, an available model, and supported isolated authentication ready. Follow installed `docs/advisor-native.md` security rules: state outside workspace; service `CODEX_HOME` explicit/private/no config.toml; no `.codex` directory at the workspace or its ancestors; no credential copying. Claude requires SDK 0.3.263/bundled CLI 2.1.263 and its own explicit authentication. A missing credential or failed preflight is a blocker, not permission to change global settings.
