...
const sdkSchema = <Schema extends z.ZodType>(schema: Schema) =>
	schema as Schema & StandardSchemaV1<z.input<Schema>, z.output<Schema>>;

export const agentStartRequest = z
	.object({
		version,
		runId: id,
  ...
  continuation: z.boolean().optional(),
	...
	if (Boolean(value.bootstrap) !== Boolean(value.resultPath)) {
			context.addIssue({
				code: "custom",
				message: "worker bootstrap and resultPath must be supplied together",
			});
	}
});
export const agentWaitRequest = z
	.object({
		version,
		runId: id,
		threadId: id,
		until: z.enum(["active", "idle"]),
	})
	.strict();
export const agentStopRequest = z.object({ version, runId: id, threadId: id }).strict();
export const wakeAuthorizeRequest = z
	.object({
		version,
		runId: id,
		threadId: id,
		logicalParentThreadId: id,
		settlementGeneration: z.string().regex(/^[a-f0-9]{64}$/u),
	})
	.strict();
export const privateRoleState = z
	.object({
		role: id,
		runDir: absolutePath,
	...
	.strict();
export const privateInitializationStatus = z.enum(["reserved", "resolving", "initialized", "stalled", "unknown"]);

export const graphSnapshot = z
	.object({
		graphId: id,
		hash: z.string().regex(/^[a-f0-9]{64}$/u),
		waves: z.array(z.object({ index: z.number().int().positive(), nodeIds: z.array(id) }).strict()),
		nodes: z.array(
			z
				.object({
					id,
					...
					artifactPath: path.optional(),
					artifact: z.string().optional(),
				})
				.strict(),
		),
	edges: z.array(z.object({ from: id, to: id }).strict()),
		launchReservations: z.array(
			z
				.object({
		runId: id,
					state: z.enum(["reserved", "bound", "failed", "resolving", "initialized", "stalled", "unknown"]),
		threadId: id.optional(),
		hostId: id,
		projectId: id,
		cwd: path,
		...
		wakeAdmissions: z.array(
			z
				.object({
					logicalParentThreadId: id,
					settlementGeneration: z.string(),
					runId: id,
					state: z.enum(["pending", "claimed", "sent", "unknown"]),
					operatorSkippedAt: z.number().int().optional(),
				})
				.strict(),
		),
	})
...
export const metaHarnessRpcContract = defineRpcContract({
	snapshot: {
		input: sdkSchema(z.object({ graphId: id }).strict()),
		output: sdkSchema(graphSnapshot),
	},
	stop: {
		input: sdkSchema(z.object({ threadId: id }).strict()),
		output: sdkSchema(z.object({ ok: z.literal(true) }).strict()),
	},
	message: {
		input: sdkSchema(z.object({ threadId: id, text: z.string().min(1).max(100_000) }).strict()),
		output: sdkSchema(z.object({ ok: z.literal(true) }).strict()),
	},
	answerBlocked: {
		input: sdkSchema(
			z
				.object({
					runId: id,
					threadId: id,
					answer: z.literal("BB-POC-CONTINUE"),
				})
				.strict(),
		),
		output: sdkSchema(z.object({ ok: z.literal(true) }).strict()),
...
export const hostRpcContract = defineRpcContract({
	...
	watchRun: {
		input: sdkSchema(z.object({ runId: id }).strict()),
		output: sdkSchema(z.object({ watching: z.literal(true), root: path }).strict()),
	},
});
