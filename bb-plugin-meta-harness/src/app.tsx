...
function MetaHarnessPanel() {
  ...
  return <main className="mh-shell">
	...
	{snapshot ? <section className="mh-panel" aria-labelledby="launches-title"><h2 id="launches-title">Launch reservations</h2>{snapshot.launchReservations.length ? snapshot.launchReservations.map((launch) => <div className="mh-launch" data-state={launch.state} key={launch.runId}><strong>{launch.state}</strong> · run {launch.runId} · {launch.threadId ? `thread ${launch.threadId} · ` : ""}host {launch.hostId} · {launch.cwd}{launch.failureReason ? <div className="mh-meta">{launch.failureReason}</div> : null}</div>) : <div className="mh-empty">No launch reservations.</div>}</section> : null}
