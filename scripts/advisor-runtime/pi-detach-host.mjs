...
import { createRoutedExecutionPort } from './agent-router.mjs';
...
export async function hostPiDetach({ stateRoot, cwd, sessionId, credentialPath, port, slots = 16, managedIdentity = null, maxLaunches = null, revision = null, keepAlive = true, routerConfigPath = undefined }) {
  ...
  const runtime = new AdvisorRuntime({ stateRoot, allowedRoots, controlPaths: [credentialPath],
    ...
    piBridge: { version: 1, portVersion: executionPort.version, principalId: principal.id, sessionId, scopes, cwd: canonicalCwd, prepare: executionPort.prepare, release: executionPort.release, managedIdentity, rootHost, maxLaunches, allowedRoots, childGrant, dynamic: Boolean(managedIdentity) || childGrant?.v === 2, readLive: adapter.readLive, revision },
  });
  try {
