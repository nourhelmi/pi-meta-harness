...
export const NATIVE_LIMITS = Object.freeze({ stderr: 65536, rpcMs: 8000 });
...
export const MAKER_DOCTRINE = 'You own this packet end to end. Inspect the relevant code, diagnose, implement, verify, inspect your diff, and commit when authorized. Work only in the assigned workspace. Never launch another agent, graph, scheduler, routine, or inter-session message. Ask through the supported host only for a real product decision, permission, credential, or external action. Return a short handoff with status, changed files, checks actually run, decisions, and anything unresolved. Never expose credentials, weaken a check, or call prose trusted verification.';
...
export class NativeSession {
  ...
  count(value) { this.bytes += Buffer.byteLength(typeof value === 'string' ? value : JSON.stringify(value));  }
  startTurn() { demand(!this.active, 'OUT_OF_ORDER'); ++this.turns; this.active = true; this.cancelled = false; this.answer = ''; }
  progress(text) {
  ...
  block(key, kind, text, respond) {
    ...
    demand(!this.pending, 'REQUEST_BOUND_OR_CONCURRENT');
  ...
  finish(status, text) {
    ...
    const output = typeof text === 'string' ? text : status === 'done' ? '' : `Native turn ${status}.`;
