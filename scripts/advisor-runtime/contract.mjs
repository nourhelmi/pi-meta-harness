...
export const MUTATIONS = ['workstream.create', 'packet.admit', 'graph.admit', 'wave.launch', 'node.launch', 'root.create', 'root.message', 'root.reply', 'root.cancel', 'root.stop', 'root.resume', 'node.reply', 'node.task', 'node.message', 'node.cancel', 'node.resume', 'team.enlist', 'team.rename', 'team.context', 'team.assign', 'team.message', 'team.retire', 'delivery.ack'];
...
export const LIMITS = Object.freeze({ connections: 32, requests: 128, waitMs: 10000, events: 128, artifact: 65536 });
...
export function text(value, _legacyMax) { demand(typeof value === 'string' && value.trim(), 'INVALID_TEXT'); }
...
export function parseEnvelope(input) {
  ...
  switch (command.op) {
    ...
    case 'wave.launch': fields(p, ['wave']); integer(p.wave, 1); break;
    ...
    case 'node.message': fields(p, ['attempt', 'handleId', 'generation', 'text']); integer(p.attempt, 1); text(p.handleId); integer(p.generation, 1); text(p.text); break;
    ...
    case 'team.assign':
      fields(p, ['attempt', 'handleId', 'generation', 'assignmentId', 'task', 'acceptance']);
      ...
      p.acceptance.forEach(item => text(item, 2048)); break;
    case 'team.message':
    ...
    case 'artifact.read': case 'log.read': fields(p, ['path', 'offset', 'maxBytes']); text(p.path, 4096); integer(p.offset); integer(p.maxBytes, 1, LIMITS.artifact); break;
  }
...
export function validatePacket(p) {
  fields(p, ['role', 'task', 'acceptance', 'cwd', 'adapter', 'model', 'thinking'], ['execution']);
  if (p.execution !== undefined) {
    ...
    fields(e.environment, ['ADVISOR_RUNTIME_DESCRIPTOR', 'PI_DETACH_RUNTIME_BRIDGE', 'ADVISOR_BRIDGE_WORKER_DIR', 'ADVISOR_RUNTIME_CANONICAL_OWNER'], ['ADVISOR_BRIDGE_CHILD_STATE', 'PATH', 'PI_CODING_AGENT_DIR', 'PI_DETACH_AGENT_PROFILES', 'CODEX_HOME', 'ADVISOR_WORKSTREAM', 'PI_DETACH_WORKER_HARNESS', 'ADVISOR_TEAM_MODE', 'AGENT_MESSAGE_DESCRIPTOR', 'AGENT_MESSAGE_CLIENT', 'AGENT_MESSAGE_CLI', 'AGENT_MESSAGE_NODE']);
...
export function validateGraph(p) {
  ...
  demand(p.topology === 'flat-root', 'UNSUPPORTED_TOPOLOGY'); integer(p.maxParallel, 1); integer(p.maxRepairLoops, 0);
  demand(Array.isArray(p.waves) && p.waves.length > 0, 'INVALID_GRAPH');
  const seen = new Map();
  ...
  for (const [node, deps] of Object.entries(p.dependencies)) {
    demand(Array.isArray(deps) && new Set(deps).size === deps.length, 'INVALID_DEPENDENCIES');
