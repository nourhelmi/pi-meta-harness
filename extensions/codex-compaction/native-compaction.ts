...
function messagesToResponseItems(model: Model<any>, messages: Message[], tools: ToolInfo[]): ResponseItem[] {
	...
	// SAFETY: pi-ai Message objects are read as JSON-shaped records; the serializer only inspects role/content fields.
				...
				if (block.type === "thinking" && typeof block.thinkingSignature === "string") {
					if (isSameProviderAndApi) {
						try {
							...
							// Malformed reasoning signature: replay the turn without its reasoning item.
						}
					}
					continue;
				}
...
/** The provider payload of the last live request for a session, captured as it was sent. */
export type LiveRequestCapture = {
	modelKey: string;
	/** Exact payload, including `input`. */
	payload: JsonObject;
	/** Branch length at capture time; entries at or beyond this index postdate the live prefix. */
	branchLength: number;
	/** Id of the last branch entry at capture time; a mismatch means the leaf moved and the prefix is stale. */
	leafEntryId?: string;
};

export type CompactionInputPlan = {
	input: ResponseItem[];
	basePayload?: JsonObject;
	source: "live-prefix" | "rebuild";
};

function liveCaptureMatchesBranch(live: LiveRequestCapture, branch: SessionEntry[]): boolean {
	if (!Array.isArray(live.payload.input)) return false;
	if (live.branchLength > branch.length) return false;
	const leafEntryId = live.branchLength > 0 ? branch[live.branchLength - 1]?.id : undefined;
	if (leafEntryId !== live.leafEntryId) return false;
	const tail = branch.slice(live.branchLength);
	return !tail.some((entry) => entry.type === "compaction" || entry.type === "branch_summary");
}

/**
 * Plan the compaction request input so its prefix is byte-identical to the last live
 * provider request: the captured `input` is reused verbatim and only the entries persisted
 * after that request are serialized and appended. The compaction call then shares the
 * conversation's prompt-cache prefix instead of re-billing the whole context. Falls back to
 * a full rebuild when no matching capture exists or the branch diverged since the capture.
 */
export function planCompactionInput(params: {
	branch: SessionEntry[];
	model: Model<any>;
	tools: ToolInfo[];
	live?: LiveRequestCapture;
	excludeLastAssistantError?: boolean;
}): CompactionInputPlan {
	const { live, branch } = params;
	const basePayload = live && live.modelKey === modelKey(params.model) ? stripInputFromPayload(live.payload) : undefined;
	const rebuild = (): CompactionInputPlan => ({
		input: effectiveInputForBranch({
			branch,
			model: params.model,
			tools: params.tools,
			excludeLastAssistantError: params.excludeLastAssistantError,
		}),
		basePayload,
		source: "rebuild",
	});
	if (!live || !basePayload || !liveCaptureMatchesBranch(live, branch)) return rebuild();

	const liveInput = live.payload.input as unknown[];
	const prefix = liveInput.filter(isResponseItem).map(cloneItem);
	if (prefix.length !== liveInput.length) return rebuild();

	let tail = branch.slice(live.branchLength);
	if (params.excludeLastAssistantError) {
		const lastAssistantIndex = branch.findLastIndex(
			(entry) => entry.type === "message" && entry.message.role === "assistant",
		);
		// The turn to drop sits inside the captured prefix; only a rebuild can remove it.
		if (lastAssistantIndex >= 0 && lastAssistantIndex < live.branchLength) return rebuild();
		if (lastAssistantIndex >= live.branchLength) {
			const tailIndex = lastAssistantIndex - live.branchLength;
			tail = tail.filter((_entry, index) => index !== tailIndex);
		}
	}
	return {
		input: [...prefix, ...entriesToResponseItems(params.model, tail, params.tools)],
		basePayload,
		source: "live-prefix",
	};
}
...
export function buildCompactionRequestBody(params: {
	basePayload?: JsonObject;
	/** Reuse the base payload's `instructions` and `tools` verbatim so the cached prefix survives. */
	preferBasePrefix?: boolean;
	model: Model<any>;
	input: ResponseItem[];
	instructions: string;
	tools?: unknown[];
	...
	const baseInstructions = params.preferBasePrefix && typeof base.instructions === "string" && base.instructions.length > 0
		? base.instructions
		: undefined;
	const baseTools = params.preferBasePrefix && Array.isArray(base.tools) && base.tools.length > 0 ? base.tools : undefined;
	...
	const body: JsonObject = {
		...
		store: false,
		stream: true,
		instructions: baseInstructions ?? params.instructions,
	...
	const tools = baseTools ?? params.tools;
...
export function buildCodexHeaders(params: {
	apiKey: string;
	/** Provider headers as Pi resolves them; `null` marks a header the provider removed. */
