...
import {
	...
	type CompactionInputPlan,
	type JsonObject,
	type LiveRequestCapture,
	type NativeCompactionDetails,
} from "./native-compaction.ts";

/** Injectable remote call so tests can assert the exact compaction request without network access. */
export type CodexCompactionOptions = {
	remote?: typeof callRemoteCompaction;
};
...
export function registerCodexCompactionExtension(
	...
	const remoteCompaction = options.remote ?? callRemoteCompaction;
	/** Last live provider request per session, reused as the compaction request's cache prefix. */
	const liveRequestBySession = new Map<string, LiveRequestCapture>();
	...
	const createNativeCheckpoint = async (params: {
		...
		const body = buildCompactionRequestBody({
			basePayload: params.plan.basePayload,
			preferBasePrefix: params.plan.source === "live-prefix",
			model: params.model,
			input: params.plan.input,
			instructions: params.ctx.getSystemPrompt(),
			tools: buildToolPayload(allTools, pi.getActiveTools()),
			sessionId,
		});
		const remote = await remoteCompaction({
		...
		return {
			details: {
				...
				replacementHistory: buildReplacementHistory(params.plan.input, remote.compactionItem),
			},
			usage: remote.usage,
		};
	};
	...
	pi.on("model_select", (_event, ctx) => {
		liveRequestBySession.delete(ctx.sessionManager.getSessionId());
		legacyCompaction = undefined;
	});
	...
	pi.on("before_provider_request", async (event, ctx) => {
		...
		// Capture the payload exactly as it leaves this hook: a later compaction reuses it as its
		// cache prefix, so it must be the bytes the provider actually saw.
		const capture = (payload: JsonObject): void => {
			liveRequestBySession.set(sessionId, {
		...
		try {
			...
			liveRequestBySession.delete(sessionId);
			ctx.abort();
			if (ctx.hasUI) {
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		try {
			...
			const plan = planCompactionInput({
				branch,
				model,
				tools: pi.getAllTools(),
				live: liveRequestBySession.get(sessionId),
