...
import {
  BbAgentStartResponseSchema as DetachStartResponseSchema,
  BbBootstrapClaimResponseSchema as DetachClaimResponseSchema,
} from "/Users/nour/Dev/pi-detach-worktrees/bb-adapter-poc/src/bb/protocol.ts";
import {
  BbBootstrapClaimResponseSchema,
  BbStartResponseSchema,
  createBbSurfaceClient,
  detectBbSurfaceContext,
} from "../extensions/bb-surface.ts";
...
test("BB surface requires the complete mutually-exclusive loopback identity", () => {
  ...
  assert.throws(() => detectBbSurfaceContext({ ...full, BB_SERVER_URL: "http://example.com" }), /loopback http/);
	assert.throws(() => detectBbSurfaceContext({ ...full, BB_SERVER_URL: "http://127.0.0.1:38886/path" }), /loopback http/);
...
test("Meta and pi-detach accept and reject identical shared response fixtures", () => {
  const start = { version: "1", runId: "run-1", threadId: "thread-1", logicalParentThreadId: "parent-1", hostId: "host-1", projectId: "project-1", environmentId: "environment-1", providerId: "pi", model: "openai/gpt", reasoning: "high", cwd: "/worktree" };
  const claim = { version: "1", role: "builder", runDir: "/advisor/run", maxTurns: 3, launchModel: "openai/gpt", launchThinking: "high", allowSubagents: false };
  ...
  for (const fixture of [claim, { ...claim, extra: true }, { ...claim, maxTurns: 0 }, { ...claim, maxTurns: 101 }, { ...claim, runDir: "relative" }]) {
    assert.equal(Value.Check(BbBootstrapClaimResponseSchema, fixture), Value.Check(DetachClaimResponseSchema, fixture));
  }
});
...
test("BB surface client uses only the fixed local route and fails closed on redirects and protocol drift", async () => {
  ...
  assert.ok(context);
  const response = { version: "1", runId: "run-1", threadId: "thread-1", logicalParentThreadId: "parent-1", hostId: "host-1", projectId: "project-1", environmentId: "environment-1", providerId: "pi", model: "openai/gpt", reasoning: "high", cwd: "/worktree" };
  ...
  const redirecting = createBbSurfaceClient(context, async () => new Response(null, { status: 302, headers: { location: "http://evil.test" } }));
  ...
  await assert.rejects(drifting.start({}), /violated protocol/);
	const oversized = createBbSurfaceClient(context, async () => new Response(new ReadableStream<Uint8Array>({
	start(controller) {
