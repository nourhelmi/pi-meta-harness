...
import { resultDeviations, validateResultArtifact } from "../scripts/advisor-core/result-artifact.mjs";
...
test("script deviations: absent or empty heading returns no summaries", () => {
  for (const markdown of ["", "# Status\nPASS", "Deviations\n- Not a heading", "## Deviations\n", "## Deviations\n\n## Files\n- Outside", "## Deviations\n-\n*\n+"]) {
    assert.deepEqual(resultDeviations(markdown), []);
  }
});

test("script deviations: first lines are trimmed, bounded and section-scoped", () => {
  const markdown = [
