...
Give a worker a self-contained task, edit boundary and falsifiable acceptance checks. Include relevant context and paths, not your whole transcript. The shipped roles are advisor, builder, checker, and browser-verifier. Own planning and synthesis as the advisor; makers investigate, plan, implement and verify within their scope, and checkers can repair findings. Use browser-verifier for focused browser checks or before/after evidence when a small economical worker adds value. Allow reasonable local obstacle resolution within the accepted outcome. Escalate changed authorization, safety or product decisions—not recoverable toolchain issues.
